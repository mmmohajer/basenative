...
export const applyClassNames = className => {
  ...
  classes.forEach(cls => {
    if (globalStyles[cls]) {
      Object.assign(styles, globalStyles[cls]);
    } else {
      try {
      ...
      try {
        ...
        console.error(
          `Error processing border radius for class ${cls}:`,
          error,
        );
      }
      ...
      try {
        if (cls.startsWith('z-')) {
          ...
          styles.zIndex = parseInt(zIndexVal, 10); // Set the z index
          styles.elevation = parseInt(zIndexVal, 10);
        }
      } catch (error) {
        console.error(`Error processing Flex Grow for class ${cls}:`, error);
      }
      ...
      try {
        ...
        console.error(`Error processing other for class ${cls}:`, error);
      }
  ...
  classes.forEach(cls => {
    if (globalStyles[cls]) {
      ...
      try {
        ...
        console.error(
          `Error processing text transform for class ${cls}:`,
          error,
        );
      }
      ...
      try {
        ...
        console.error(
          `Error processing text alignment for class ${cls}:`,
          error,
        );
      }
    }
